...
import org.apache.spark.sql.execution.dita.util.{HashMapParam, NormalFileWriter}
...
object TrajectorySimilarityWithThresholdAlgorithms {
  ...
  def localJoin(partitionIter: Iterator[PackedPartition],
           ...
           threshold: Double): Iterator[(Trajectory, Trajectory, Double, Int, Int)] = {
    ...
    val answerPairs = queryTrajectories.flatMap(query => {
      val indexCandidates = localIndex.getCandidates(query, distanceFunction, threshold, packedPartition.id)

//      System.out.println("Packed Partition ID " +packedPartition.id+"INdex cand "+indexCandidates.size)
      val mbrCandidates = indexCandidates.filter(t => query.getExtendedMBR.contains(t._1.getMBR))
      mbrCandidates.map(t => (t._1, distanceFunction.evalWithTrajectory(query, t._1, threshold),t._2))
        .filter(t => t._2 <= threshold)
        .map(t => (query, t._1, t._2, packedPartition.id, t._3))
    })

    //answerPairs.foreach(x => println("CHECK;;; "+packedPartition.id+" ; " + x._5))
    answerPairs.foreach(x => NormalFileWriter.instance().Write(packedPartition.id+" ; " + x._5))
  ...
  object DistributedSearch {
    ...
    def search(sparkContext: SparkContext, query: Trajectory, trieRDD: TrieRDD,
               distanceFunction: TrajectorySimilarity,
               threshold: Double): RDD[(Trajectory, Double, Int, Int)] = {
      ...
      //val candidatePartitions = List(21)
      //System.out.println(candidatePartitions)
      System.out.println(candidatePartitions)
    ...
    //modified the search using learned index
    def search(sparkContext: SparkContext, query: Trajectory, trieRDD: TrieRDD,
                           distanceFunction: TrajectorySimilarity,
                           threshold: Double): RDD[(Trajectory, Double, Int, Int)] = {
      val bQuery = sparkContext.broadcast(query)

      val candidatePartitions = LearnedModel.getInstance().predict(query, threshold)
    ...
    RDD[(Trajectory, Trajectory, Double)] = {
      ...
      val leftPartitionCost = leftTrieRDD.packedRDD.zipPartitions(sampledPartitionedRightCandidatesRDD) { case (partitionIter, trajectoryIter) =>
        ...
        val candidatesCount = trajectoryIter
          .map(t => localTrieIndex.getCandidates(t, distanceFunction, threshold, packedPartition.id).size)
          .sum
      ...
      if (balancingPartitions.isEmpty) {
        //modified later!
        normalAnswerRDD.map(x => (x._1,x._2,x._3))
      } else {
        ...
        val balancingAnswerRDD = balancingLeftRDD
          .zipPartitions(balancingPartitionedRightCandidatesRDD) { case (partitionIter, trajectoryIter) =>
            ...
            //modified later!
        normalAnswerRDD.union(balancingAnswerRDD).map(x => (x._1,x._2,x._3))
      }
    }
  }
      ...
      leftTrieRDD.packedRDD.zipPartitions(partitionedSampledRightCandidatesRDD) { case (partitionIter, trajectoryIter) =>
        ...
        trajectoryIter.foreach(t => {
          ...
          compCost.add((s"R$rightPartitionIdx-L${packedPartition.id}",
            localTrieIndex.getCandidates(t, distanceFunction, threshold, packedPartition.id).size))
        })
