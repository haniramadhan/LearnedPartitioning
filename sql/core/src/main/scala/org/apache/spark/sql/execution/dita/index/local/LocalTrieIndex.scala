...
case class LocalTrieIndex(partitioner: LocalTriePartitioner) extends LocalIndex {
  def getCandidates(key: Trajectory, distanceFunction: TrajectorySimilarity, threshold: Double, globalID: Int): List[(Trajectory,Int)] = {
    partitioner.getCandidates(key, distanceFunction, threshold, 0.0, globalID, -1).filter(_._2 <= threshold).map(t => (t._1,t._3))
  }
  ...
  def getCandidatesWithDistances(key: Trajectory, distanceFunction: TrajectorySimilarity, threshold: Double): List[(Trajectory, Double)] = {
    partitioner.getCandidates(key, distanceFunction, threshold, 0.0, 0, -1).map(t => (t._1,t._2))
  }
}
